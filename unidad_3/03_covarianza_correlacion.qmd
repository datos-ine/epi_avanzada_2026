...
datos |>
  ...
  geom_vline(xintercept = mean(datos$x), linetype = "dashed") +
  geom_hline(yintercept = mean(datos$y), linetype = "dashed") +

  # etiquetas
  labs(x = "Actividad física (min/semana)", y = "Capacidad aeróbica") +
...
datos |>
  ...
  # etiquetas
  labs(
    x = "Consumo de Alcohol (gramos/semana)",
...
datos |>
  ...
  # etiquetas
  labs(x = "Horas de Sueño", y = "Nivel de Colesterol (mg/dL)") +
...
datos |>
  ...
  annotate("text", x = 10, y = 85, label = paste0("R² = ", cor)) +
...
datos |>
  ...
  # añade coeficiente de correlación
  annotate("text", x = 10, y = 100, label = paste0("R² = ", cor)) +
...
datos |>
  ...
  # añade coeficiente de correlación
  annotate("text", x = -7.5, y = 85, label = paste0("R² = ", cor)) +
...
Podemos generar diagramas de dispersión para cada par de variables numéricas usando la función `ggpairs()` del paquete `GGally` [@schloerke2010]:
