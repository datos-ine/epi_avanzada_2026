...
# Datos
tibble(
  `Estructura del DAG` = c("X --> M --> Y", "X <-- Z --> Y", "X --> C <-- Y"),
  Nombre = c("Cadena (mediación)", "Bifurcación (confusión)", "Colisionador")
) |>
...
Es consecuencia generalmente de un sesgo de selección y se suele manifestar en estudios de base hospitalaria (sesgo de Berkson), en estudios longitudinales, si el abandono depende de la exposición y de covariables del desenlace (pérdida no aleatoria) y en estudios con sesgo de supervivencia (análisis de los supervivientes). Ver @holmberg2022
...
# Datos
tibble(
  Variable = c("Confusora", "Mediadora", "Colisionadora", "Interacción"),
  `Estructura DAG` = c(
    "X <-- Z --> Y",
    "X --> M --> Y",
    "X --> C <-- Y",
  ...
  Ajuste = c("Sí", "Depende del objetivo", "No", "Incluir como interaccón"),
  `Si no ajustás` = c(
    "Sesgo de confusión",
